...
class GraphWin(tk.Canvas):
    ...
    def customSetCoords(self, x1, y1, x2, y2, pointList):
        self.trans = Transform(self.width, self.height, x1, y1, x2, y2)
        self.optimizedRedraw(pointList)
    
    def secondCustomSetCoords(self, x1, y1, x2, y2, partitioningDict):
        self.trans = Transform(self.width, self.height, x1, y1, x2, y2)
        rektsThatCameraIsIn = []
        rektsThatCameraIsIn.append("other")
        x = (x1 + x2)/2
        y = (y1 + y2)/2
        x = x // (self.width/2)
        y =  y // (self.height/2)
        for dx in range(-1,2):
            for dy in range(-1,2):
                p = Point((x + dx),(y + dy))
                rektsThatCameraIsIn.append(p.__repr__())
        
        for item in self.items[:]:
            item.undraw()
        for index in rektsThatCameraIsIn:
            for item in partitioningDict[index]:
                item.draw(self)
        self.update()
    ...
    def optimizedRedraw(self,pointList):
        for item in self.items[:]:
            if type(item) == Point:
                pass
            else:
                ...
                item.draw(self)
        for i in pointList:
            if self.trans.xbase < i.getX() < (self.trans.xbase + self.trans.xspan):
                i.undraw()
                i.draw(self)
            elif (self.trans.ybase - self.trans.yspan) < i.getY() < self.trans.ybase:
                i.undraw()
                i.draw(self)
            else:
                i.undraw()
...
class GraphicsObject:
    ...
    def __init__(self, options):
        # 
        # 
        #  is a list of strings indicating which options are
